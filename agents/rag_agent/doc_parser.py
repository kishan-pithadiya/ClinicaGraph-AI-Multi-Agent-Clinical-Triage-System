...
class MedicalDocParser:
    ...
    def __init__(self):
        ...
        if not DOCLING_AVAILABLE:
            self.logger.warning("Docling is not installed. PDF ingestion will be disabled (pre-indexed vector store will be used).")
        else:
    ...
    def parse_document(
        ...
        if not DOCLING_AVAILABLE:
            raise ImportError(
                "The 'docling' package is required for ingestion of new PDF documents. "
                "Install it using: pip install docling docling-core. "
                "Note: Pre-indexed vector embeddings in data/qdrant_db are already available for inference without docling."
            )
